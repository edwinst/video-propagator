...
\begin{itemize}
\item \alert<1>{... at large imaginary values are neglibible}
...
%:     my $plot = '';
%:     if ($t > 0.1) {
%:         $plot .= q{ --append-plot '"tmp/bessel-FUNCTION.dat" using 2:5 with lines lc rgb "black" lt 2 lw 3 title "2*m*K1(mr)"' };
%:     }
%:     if ($t > 0.2) {
%:         $plot .= q{ --pre-plot 'm = 1' --append-plot 'sqrt(2*pi*m/x)*exp(-m*x) with lines lc rgb "black" lt 3' };
%:     }
%:     if ($t > 0.3) {
%:         $plot .= q{ --pre-plot 'set label "zooming..." at 5,8 font "serif,20"' };
%:     }
