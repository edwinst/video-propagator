...
\pause
More carefully we can consider the fields as \alert{distributions} which must be \alert{smeared} by test functions.
\end{frame}


\begin{frame}
For simplicity, let's apply a Gaussian smearing $e^{-\frac{(x' - x)^2}{2\sigma^2}}$ only along $\vr$.
...
%: animate(12, sub {
%:     my $sigma = interpol_xaby(3, 0.2, 0.3, 0.1, 0, 0.2, 0.01, 0.1, $_[0]);
...
\begin{frame}
For this pushed contour, the smearing makes no difference...
\end{frame}
